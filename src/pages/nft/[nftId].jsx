...
function HistoryItem({ event }) {
  const getEventIcon = () => {
    switch (event.event) {
      case 'minted': return <Sparkles className="w-4 h-4 text-purple-400" />;
      case 'listed': return <Tag className="w-4 h-4 text-green-400" />;
      case 'unlisted': return <X className="w-4 h-4 text-gray-400" />;
      ...
      case 'bid': return <Gavel className="w-4 h-4 text-pink-400" />;
  ...
  return (
    <div className="flex items-center gap-3 py-3 border-b border-gray-700 last:border-0">
      ...
      <div className="flex-1">
        <p className="text-white text-sm">{getEventText()}</p>
          ...
          <div>
            <div className="relative rounded-2xl overflow-hidden bg-gray-800">
              ...
              <div className="absolute top-4 left-4 flex gap-2">
                ...
                {nft.hasUnlockable && (
                  <span className="px-3 py-1 bg-purple-500 text-white text-sm font-bold rounded-lg flex items-center gap-1">
              ...
              <div className="absolute top-4 right-4 flex gap-2">
                ...
                {isOwner && (
                  <div className="relative">
                    ...
                    {showMenu && (
                      <div className="absolute right-0 top-full mt-2 bg-gray-800 rounded-xl border border-gray-700 overflow-hidden z-10 min-w-[160px]">
                        {nft.isListed ? (
                          <button
                            onClick={handleUnlist}
                            className="w-full px-4 py-3 text-left text-gray-300 hover:bg-gray-700 flex items-center gap-2"
                          >
                            <X className="w-4 h-4" />
                            Remove from sale
                          </button>
                        ...
                        <Link href={`/nft/${nftId}/edit`}>
                          <button className="w-full px-4 py-3 text-left text-gray-300 hover:bg-gray-700 flex items-center gap-2">
          ...
          <div className="space-y-6">
            ...
            {nft.collection && (
              <Link href={`/nft/collection/${nft.collection._id}`}>
                <p className="text-purple-400 hover:text-purple-300 flex items-center gap-1">
                  {nft.collection.name}
              ...
              {nft.isListed ? (
                <>
                    ...
                    <div className="flex gap-3">
                      {nft.listingType === 'fixed' ? (
                        <button
                          ...
                          className="flex-1 py-4 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-xl font-bold hover:opacity-90 transition-opacity flex items-center justify-center gap-2"
                        >
                  ...
                  {isOwner && (
                    <button
                      onClick={handleUnlist}
                      className="w-full py-4 bg-gray-700 text-white rounded-xl font-medium hover:bg-gray-600 transition-colors"
                    >
                      Remove from Sale
                    </button>
                  )}
            ...
            <div>
              ...
              <div className="bg-gray-800/50 rounded-xl p-4 space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-white">{nft.tokenId || 'Pending'}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-white">{nft.royaltyPercentage}%</span>
                </div>
              </div>
            </div>
          ...
          <div className="bg-gray-900 rounded-2xl p-6 max-w-md w-full border border-purple-500/20">
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                ...
                <p className="text-gray-400">@{nft.creator?.username}</p>
              </div>
            </div>
            ...
            <div className="bg-gray-800 rounded-xl p-4 mb-6">
              <div className="flex justify-between mb-2">
                ...
                <span className="text-white">{nft.listingPrice} CYBEV</span>
              </div>
              <div className="flex justify-between mb-2">
                <span className="text-gray-400">Platform Fee (2.5%)</span>
                <span className="text-white">{(nft.listingPrice * 0.025).toFixed(2)} CYBEV</span>
              </div>
              <div className="flex justify-between pt-2 border-t border-gray-700">
                <span className="text-white font-medium">Total</span>
                <span className="text-white font-bold">{(nft.listingPrice * 1.025).toFixed(2)} CYBEV</span>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 rounded-2xl p-6 max-w-md w-full border border-purple-500/20">
            ...
            <p className="text-gray-400 mb-4">
              Current highest bid: <span className="text-white font-bold">{nft.highestBid || nft.listingPrice} CYBEV</span>
            </p>

            <div className="mb-6">
              <label className="block text-gray-400 mb-2">Your Bid</label>
              <div className="relative">
                <input
                  type="number"
                  value={bidAmount}
      ...
      {showListModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 rounded-2xl p-6 max-w-md w-full border border-purple-500/20">
            ...
            <div className="mb-4">
              <label className="block text-gray-400 mb-2">Sale Type</label>
              <div className="grid grid-cols-2 gap-3">
                <button
                  onClick={() => setListType('fixed')}
                  className={`p-3 rounded-xl border transition-colors ${
                    listType === 'fixed' ? 'border-purple-500 bg-purple-500/20' : 'border-gray-700'
                  }`}
                >
                ...
                <button
                  ...
                  className={`p-3 rounded-xl border transition-colors ${
                    listType === 'auction' ? 'border-purple-500 bg-purple-500/20' : 'border-gray-700'
                  }`}
                >
                  <p className="text-white font-medium">Auction</p>
                </button>
              </div>
            </div>
