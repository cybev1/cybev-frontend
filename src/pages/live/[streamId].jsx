...
export default function LiveStreamPage() {
  ...
  return (
    <>
      ...
      <div ref={containerRef} className="min-h-screen bg-gray-50 flex flex-col lg:flex-row">
        {/* Video Section */}
        <div className="flex-1 flex flex-col">
          ...
          <div className="bg-white px-4 py-3 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-gray-900 font-semibold truncate max-w-xs">{stream.title}</h1>
                <p className="text-gray-500 text-sm">{stream.streamer?.name || stream.streamer?.username}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              {isStreamer && isLive && (
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-red-600 text-gray-900 rounded-lg hover:bg-red-700 disabled:opacity-50 font-semibold"
                >
                  {endingStream ? (
          ...
          {/* Video Player */}
          <div className="relative bg-gray-900 aspect-video lg:aspect-auto lg:flex-1">
            {hasHlsStream && !isEnded ? (
              <div className="absolute inset-0">
                ...
                {!hlsReady && !videoError && (
                  <div className="absolute inset-0 flex items-center justify-center bg-gray-900/80">
                    <div className="text-center">
                      ...
                      <p className="text-gray-900 font-medium">Connecting to stream...</p>
                      <p className="text-gray-500 text-sm mt-2">
                        {hlsRetryCount > 0 ? `Attempt ${hlsRetryCount + 1}/10...` : 'Please wait'}
                      </p>
                      {hlsRetryCount >= 3 && (
                        <button onClick={retryConnection} className="mt-4 px-4 py-2 bg-purple-600 text-gray-900 rounded-lg">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center bg-gray-900/80">
                    <div className="text-center max-w-md px-4">
                      ...
                      <div className="flex gap-2 justify-center">
                        <button onClick={retryConnection} className="px-6 py-2 bg-purple-600 text-gray-900 rounded-lg">
                          <RefreshCw className="w-4 h-4 inline mr-2" />Retry
                        </button>
                        <Link href="/tv">
                          <button className="px-6 py-2 bg-gray-100 text-gray-900 rounded-lg">Back to TV</button>
                        </Link>
                      </div>
                    </div>
                  </div>
                ...
                {hlsReady && isMuted && (
                  <div className="absolute inset-0 flex items-center justify-center cursor-pointer" onClick={() => setIsMuted(false)}>
                    <div className="bg-gray-900/60 px-4 py-2 rounded-lg flex items-center gap-2 text-gray-900">
                ...
                <div className="absolute top-4 left-4 flex items-center gap-3">
                  {isLive && (
                    <div className="flex items-center gap-2 px-3 py-1.5 bg-red-600 rounded-lg text-gray-900 text-sm font-bold">
                      <div className="w-2 h-2 bg-white rounded-full animate-pulse" />
                      LIVE
                    </div>
                  )}
                  <div className="flex items-center gap-1 px-3 py-1.5 bg-gray-900/50 rounded-lg text-gray-900 text-sm">
                ...
                <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <button onClick={() => setIsMuted(!isMuted)} className="p-2 bg-gray-900/50 rounded-full text-gray-900 hover:bg-gray-900/70">
                    ...
                    {QUICK_REACTIONS.map(emoji => (
                      <button
                        ...
                        className="w-8 h-8 bg-gray-900/50 rounded-full flex items-center justify-center text-sm hover:bg-gray-900/70 hover:scale-110 transition"
                      >
                        {emoji}
                      </button>
                    ))}
                  </div>
                  <button onClick={toggleFullscreen} className="p-2 bg-gray-900/50 rounded-full text-gray-900 hover:bg-gray-900/70">
                ...
                <div className="text-center">
                  ...
                  {stream.recordingUrl && (
                    <a href={stream.recordingUrl} target="_blank" rel="noopener noreferrer">
                      <button className="mt-4 px-6 py-2 bg-purple-600 text-gray-900 rounded-lg hover:bg-purple-700">Watch Recording</button>
                    </a>
                  )}
                  <Link href="/tv">
                    <button className="mt-2 px-6 py-2 bg-gray-100 text-gray-900 rounded-lg hover:bg-gray-600 block mx-auto">Back to TV</button>
                  </Link>
                </div>
              </div>
            ) : (
              <div className="absolute inset-0 flex items-center justify-center bg-white">
                <div className="text-center">
                  ...
                  <p className="text-gray-900">Waiting for stream...</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="bg-white px-4 py-3 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Link href={`/profile/${stream.streamer?.username || stream.streamer?._id}`}>
                <div className="w-12 h-12 rounded-full bg-purple-600 flex items-center justify-center text-gray-900 font-bold overflow-hidden">
              ...
              <div>
                <p className="text-gray-900 font-semibold">{stream.streamer?.name || stream.streamer?.username}</p>
                <p className="text-gray-500 text-sm">{stream.streamer?.followers?.length || 0} followers</p>
              </div>
            </div>
            ...
            {user && !isStreamer && (
              <button
                ...
                className={`px-4 py-2 rounded-lg font-medium flex items-center gap-2 ${
                  isFollowing ? 'bg-gray-100 text-gray-900' : 'bg-purple-600 text-gray-900 hover:bg-purple-700'
                }`}
              >
        ...
        {/* Chat Section */}
        <div className={`lg:w-96 bg-white flex flex-col ${showChat ? '' : 'hidden lg:flex'}`}>
            ...
            {messages.length === 0 ? (
              ...
              messages.map((msg, i) => (
                <div key={msg._id || i} className="flex gap-2">
                  <div className="w-8 h-8 rounded-full bg-purple-600 flex-shrink-0 flex items-center justify-center text-gray-900 text-xs overflow-hidden">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="text-purple-600 text-sm font-medium">{msg.user?.name || 'User'}</span>
          ...
          <div className="p-4 border-t border-gray-200">
            <div className="flex gap-2">
              <input
                ...
                disabled={!user}
                className="flex-1 px-4 py-2 bg-gray-100 text-gray-900 rounded-full placeholder-gray-400 disabled:opacity-50"
              />
              <button
                onClick={sendMessage}
                disabled={!user || !newMessage.trim() || sendingMessage}
                className="p-2 bg-purple-600 text-gray-900 rounded-full hover:bg-purple-700 disabled:opacity-50"
              >
        ...
        <div className="fixed inset-0 bg-gray-900/70 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 max-w-md w-full">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-bold text-gray-900">Share Stream</h3>
            ...
            <div className="space-y-3">
              <button onClick={copyLink} className="w-full flex items-center gap-3 px-4 py-3 bg-gray-700 rounded-lg text-gray-900 hover:bg-gray-600">
                <LinkIcon className="w-5 h-5" />Copy Link
              </button>
              <button onClick={shareTwitter} className="w-full flex items-center gap-3 px-4 py-3 bg-blue-600 rounded-lg text-gray-900 hover:bg-blue-700">
                <Twitter className="w-5 h-5" />Share on Twitter
              </button>
              <button onClick={shareFacebook} className="w-full flex items-center gap-3 px-4 py-3 bg-blue-800 rounded-lg text-gray-900 hover:bg-blue-900">
