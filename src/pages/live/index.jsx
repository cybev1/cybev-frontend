    ...
    <Link href={`/live/${stream._id || stream.id}`}>
      <div className={`group cursor-pointer ${featured ? 'col-span-2 row-span-2' : ''}`}>
        <div className="relative overflow-hidden rounded-xl">
          ...
          <div className={`relative ${featured ? 'aspect-video' : 'aspect-video'}`}>
            ...
            <div className="absolute top-3 left-3 flex items-center gap-2">
              <span className="px-2 py-1 bg-red-500 text-gray-900 text-xs font-bold rounded flex items-center gap-1">
                <span className="w-2 h-2 bg-white rounded-full animate-pulse"></span>
                LIVE
              </span>
              <span className="px-2 py-1 bg-gray-900/60 text-gray-900 text-xs rounded flex items-center gap-1">
            ...
            <div className="absolute inset-0 bg-gray-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
              <Play className="w-16 h-16 text-gray-900" />
            </div>
          </div>
        </div>
        ...
        <div className="mt-3 flex gap-3">
          <div className="w-10 h-10 bg-gradient-to-br from-purple-500 to-pink-500 rounded-full flex items-center justify-center text-gray-900 font-bold flex-shrink-0">
            {stream.streamer?.name?.[0] || 'S'}
          </div>
          <div className="flex-1 min-w-0">
            <h3 className="text-gray-900 font-medium truncate group-hover:text-purple-600 transition-colors">
              {stream.title || 'Untitled Stream'}
            </h3>
            <p className="text-gray-500 text-sm truncate">{stream.streamer?.name || 'Anonymous'}</p>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-500 mt-1">Watch and interact with live content creators</p>
          </div>

          <button
            onClick={handleStartStream}
            className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-red-500 to-pink-500 text-gray-900 rounded-xl font-medium hover:opacity-90 transition-opacity"
          >
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-white/50 border border-gray-200 rounded-xl pl-10 pr-4 py-3 text-gray-900 placeholder-gray-400 focus:border-purple-500 focus:outline-none"
            />
          </div>
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`p-3 rounded-xl transition-colors ${viewMode === 'grid' ? 'bg-purple-500 text-white' : 'bg-white text-gray-500'}`}
            >
              <Grid className="w-5 h-5" />
            </button>
            <button
              onClick={() => setViewMode('list')}
              className={`p-3 rounded-xl transition-colors ${viewMode === 'list' ? 'bg-purple-500 text-white' : 'bg-white text-gray-500'}`}
            >
              ...
              <div className="relative rounded-2xl overflow-hidden group cursor-pointer">
                <div className="aspect-[21/9] relative">
                  ...
                  <div className="absolute top-4 left-4 flex items-center gap-2">
                    <span className="px-3 py-1.5 bg-red-500 text-gray-900 text-sm font-bold rounded-lg flex items-center gap-2">
                      <span className="w-2 h-2 bg-white rounded-full animate-pulse"></span>
                      LIVE
                    </span>
                    <span className="px-3 py-1.5 bg-gray-900/60 text-gray-900 text-sm rounded-lg flex items-center gap-2">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6">
                    <div className="flex items-end justify-between">
                      <div className="flex items-center gap-4">
                        <div className="w-16 h-16 bg-gradient-to-br from-purple-500 to-pink-500 rounded-full flex items-center justify-center text-gray-900 text-2xl font-bold border-4 border-gray-900">
                          {featuredStream.streamer?.name?.[0] || 'S'}
                        </div>
                        <div>
                          <h3 className="text-2xl font-bold text-gray-900 mb-1">{featuredStream.title}</h3>
                          <p className="text-gray-600">{featuredStream.streamer?.name}</p>
                          <p className="text-gray-500 text-sm">{featuredStream.category}</p>
                        </div>
                      </div>
                      
                      <button className="px-6 py-3 bg-red-500 text-gray-900 rounded-xl font-medium hover:bg-red-600 transition-colors flex items-center gap-2">
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <h3 className="text-xl font-bold text-gray-900 mb-2">No streams found</h3>
            <p className="text-gray-500 mb-6">Be the first to go live!</p>
            <button
              onClick={handleStartStream}
              className="px-6 py-3 bg-gradient-to-r from-red-500 to-pink-500 text-gray-900 rounded-xl font-medium"
            >
              Start Streaming
            </button>
          </div>
          ...
          <>
            <h2 className="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2">
              ...
              Live Now
              <span className="text-sm font-normal text-gray-500">({filteredStreams.length} streams)</span>
            </h2>
