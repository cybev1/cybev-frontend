...
// PURPOSE: Post detail page with Open Graph meta tags
// ============================================
...
function ShareModal({ isOpen, onClose, url, text, image }) {
  ...
  const shareLinks = [
    {
      ...
      action: () => { navigator.clipboard.writeText(url); alert('Link copied!'); onClose(); },
      color: 'bg-gray-600'
    },
    {
      name: 'Twitter / X',
      ...
      action: () => window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent(url)}&hashtags=CYBEV,Web3`),
      color: 'bg-black'
    },
    {
      name: 'Facebook',
    ...
    <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-gray-800 rounded-2xl p-6 max-w-md w-full" onClick={e => e.stopPropagation()}>
        ...
        <div className="bg-gray-900 rounded-xl p-4 mb-6">
          ...
          <p className="text-gray-400 text-xs truncate mt-2">{url}</p>
        </div>
...
export default function PostDetail() {
  ...
  const getMetaDescription = () => {
    if (!post) return 'View this post on CYBEV - The Web3 Content Platform';
    return (post.content || '').substring(0, 160);
  };

  const getMetaImage = () => {
    const images = getImages();
    return images[0] || 'https://cybev.io/og-default.png';
  };
  ...
  const getCanonicalUrl = () => {
    ...
    return `https://cybev.io/post/${postId}`;
  };
  ...
  const handleReaction = async (type) => {
    try {
      const token = localStorage.getItem('token') || localStorage.getItem('cybev_token');
      await api.post(`/api/posts/${post._id}/like`, { reaction: type }, {
  ...
  const handleComment = async () => {
    ...
    try {
      ...
      const response = await api.post(`/api/posts/${post._id}/comment`, { content: newComment }, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  return (
    <AppLayout>
      <Head>
        {/* Primary Meta Tags */}
        <title>{authorName}'s post - CYBEV</title>
        <meta name="title" content={`${authorName}'s post on CYBEV`} />
        ...
        <meta property="og:title" content={`${authorName}'s post on CYBEV`} />
        ...
        <meta property="twitter:title" content={`${authorName}'s post on CYBEV`} />
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        <div className="bg-gray-800/50 rounded-2xl border border-purple-500/20 overflow-hidden">
          ...
          {/* Images */}
          {images.length > 0 && (
            <div className={`grid gap-0.5 ${images.length === 1 ? '' : images.length === 2 ? 'grid-cols-2' : 'grid-cols-2'}`}>
              {images.slice(0, 4).map((img, i) => (
                <div
                  key={i}
                  className={`cursor-pointer overflow-hidden ${images.length === 1 ? '' : 'aspect-square'} ${images.length === 3 && i === 0 ? 'row-span-2' : ''}`}
                  onClick={() => { setGalleryIndex(i); setShowGallery(true); }}
                >
                  <img
                    src={img}
                    alt=""
                    className={`w-full h-full object-cover hover:opacity-95 transition-opacity ${images.length === 1 ? 'max-h-[500px]' : ''}`}
                  />
                  {i === 3 && images.length > 4 && (
          ...
          {/* Stats */}
          <div className="px-4 py-3 flex items-center justify-between text-gray-400 text-sm border-t border-gray-700/50">
          ...
          {/* Action Buttons */}
          <div className="px-4 py-3 flex items-center justify-around border-t border-gray-700/50">
            {/* Like */}
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-lg text-gray-400 hover:bg-gray-700/50"
            >
            ...
            <div className="flex gap-3 mb-4">
              ...
              <div className="flex-1 flex gap-2">
                <input
                  ...
                  className="flex-1 bg-gray-800 border border-gray-700 rounded-full px-4 py-2 text-white focus:border-purple-500 focus:outline-none"
                />
                <button
              ...
              {comments.length === 0 ? (
                ...
                comments.map((comment, idx) => (
                  <div key={comment._id || idx} className="flex gap-3">
                    <div className="w-9 h-9 bg-gray-700 rounded-full flex-shrink-0 flex items-center justify-center text-white text-xs font-bold">
