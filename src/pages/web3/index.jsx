        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
              ...
              <p className="text-gray-500 mt-1">Manage your wallet, stake tokens, and view NFTs</p>
            </div>
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium whitespace-nowrap transition-all ${
                  activeTab === tab.id
                    ...
                    : 'bg-white/50 text-gray-500 hover:text-gray-900 hover:bg-gray-100/50'
                }`}
              >
          ...
          {activeTab === 'overview' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-gradient-to-br from-purple-900/50 to-pink-900/50 rounded-xl p-6 border border-gray-200">
                  ...
                  {isConnected && (
                    <div className="bg-gray-900/20 rounded-lg p-3">
                      <p className="text-gray-500 text-xs mb-1">Address</p>
                      <p className="text-gray-900 font-mono text-sm truncate">{wallet}</p>
                      <p className="text-gray-500 text-xs mt-1">{chain.name}</p>
                    </div>
                  )}
                </div>

                <div className="bg-white/30 rounded-xl p-6 border border-gray-200">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-12 h-12 bg-yellow-500/20 rounded-xl flex items-center justify-center">
                      <Coins className="w-6 h-6 text-yellow-400" />
                    </div>
                    <div>
                      <p className="text-gray-500 text-sm">CYBEV Balance</p>
                      <p className="text-2xl font-bold text-gray-900">{parseFloat(tokenBalance).toLocaleString()}</p>
                    </div>
                  </div>
                ...
                <div className="bg-white/30 rounded-xl p-6 border border-gray-200">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <p className="text-gray-500 text-sm">{chain.symbol} Balance</p>
              ...
              <div className="bg-white/30 rounded-xl p-6 border border-gray-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <button
                    ...
                    className="flex items-center gap-3 p-4 bg-purple-500/20 hover:bg-purple-500/30 border border-gray-200 rounded-xl transition-colors"
                  >
                    <Lock className="w-8 h-8 text-purple-600" />
                    <div className="text-left">
                      <p className="text-gray-900 font-medium">Stake Tokens</p>
                      <p className="text-gray-500 text-sm">Earn up to 25% APY</p>
                    </div>
                  ...
                  <Link href="/studio/ai-blog">
                    <button className="w-full flex items-center gap-3 p-4 bg-green-500/20 hover:bg-green-500/30 border border-green-500/30 rounded-xl transition-colors">
                      ...
                      <div className="text-left">
                        <p className="text-gray-900 font-medium">Create & Mint</p>
                        <p className="text-gray-500 text-sm">Write blog → Mint NFT</p>
                      </div>
                  ...
                  >
                    ...
                    <div className="text-left">
                      <p className="text-gray-900 font-medium">My NFTs</p>
                      <p className="text-gray-500 text-sm">{nfts.length} minted</p>
                    </div>
                    <ArrowRight className="w-5 h-5 text-pink-600 ml-auto" />
                  </button>
                </div>
              </div>
              ...
              {nfts.length > 0 && (
                <div className="bg-white/30 rounded-xl p-6 border border-gray-200">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-lg font-semibold text-gray-900">Recent NFTs</h2>
                    <button
                      onClick={() => setActiveTab('nfts')}
                      className="text-purple-600 text-sm hover:text-purple-600"
                    >
                      View All →
                    </button>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {nfts.slice(0, 3).map((nft) => (
                      <div key={nft._id} className="bg-gray-50 rounded-lg p-4">
                        ...
                        <p className="text-gray-900 font-medium truncate">{nft.metadata?.name}</p>
            ...
            <div className="space-y-6">
              <div className="bg-white/30 rounded-xl p-6 border border-gray-200">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <span className="px-3 py-1 bg-purple-500/20 text-purple-600 rounded-full text-sm">
                    {nfts.length} NFTs
                  </span>
                </div>
                ...
                {loadingNFTs ? (
                  ...
                  <div className="text-center py-12">
                    ...
                    <Link href="/studio/ai-blog">
                      <button className="px-6 py-2 bg-purple-600 hover:bg-purple-700 text-gray-900 rounded-lg transition-colors">
                        Create Blog
                      </button>
                    </Link>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {nfts.map((nft) => (
                      <div key={nft._id} className="bg-gray-50 rounded-xl overflow-hidden border border-gray-200 hover:border-purple-500/50 transition-colors">
                        {nft.blog?.featuredImage ? (
                          <img
                            src={nft.blog.featuredImage}
                            alt={nft.metadata?.name}
                        ...
                        <div className="p-4">
                          <h3 className="text-gray-900 font-medium mb-2 line-clamp-2">{nft.metadata?.name}</h3>
                          <div className="flex items-center justify-between text-sm">
                            <span className="text-gray-500">Token #{nft.tokenId?.slice(-8)}</span>
                            <span className="text-purple-600">{new Date(nft.mintedAt).toLocaleDateString()}</span>
                          </div>
                          <div className="flex gap-2 mt-4">
                            <Link href={`/blog/${nft.blog?._id}`} className="flex-1">
                              <button className="w-full px-3 py-2 bg-gray-700 hover:bg-gray-600 text-gray-900 rounded-lg text-sm transition-colors">
                                View Blog
                              </button>
                            </Link>
                            <a
