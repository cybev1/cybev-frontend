      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 bg-pink-500 rounded-2xl flex items-center justify-center"><Bell className="w-7 h-7 text-white" /></div>
            <div><h1 className="text-2xl font-bold text-white">Push Notifications</h1><p className="text-gray-400">Send notifications to users</p></div>
          </div>
          <button onClick={() => setShowComposer(true)} className="flex items-center gap-2 px-4 py-2 bg-purple-500 text-white rounded-lg hover:bg-purple-600">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-gray-800/50 rounded-xl border border-purple-500/20 p-4 text-center">
          ...
          <div className="bg-gray-800/50 rounded-xl border border-purple-500/20 p-4 text-center">
            ...
            <p className="text-2xl font-bold text-white">{((stats.delivered / stats.totalSent) * 100).toFixed(1)}%</p>
            <p className="text-gray-400 text-sm">Delivery Rate</p>
          </div>
          <div className="bg-gray-800/50 rounded-xl border border-purple-500/20 p-4 text-center">
            <Target className="w-6 h-6 text-pink-400 mx-auto mb-2" />
            <p className="text-2xl font-bold text-white">{((stats.clicked / stats.delivered) * 100).toFixed(1)}%</p>
        ...
        {showComposer && (
          <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
            <div className="bg-gray-800 rounded-2xl max-w-lg w-full p-6">
              <h3 className="text-xl font-bold text-white mb-6">Create Notification</h3>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" value={newNotification.title} onChange={(e) => setNewNotification({ ...newNotification, title: e.target.value })}
                    placeholder="Notification title" className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-xl text-white focus:border-purple-500 focus:outline-none" />
                </div>
                ...
                <div>
                  ...
                  <textarea value={newNotification.body} onChange={(e) => setNewNotification({ ...newNotification, body: e.target.value })}
                    placeholder="Notification message" rows={3} className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-xl text-white focus:border-purple-500 focus:outline-none resize-none" />
                </div>
                ...
                <div>
                  ...
                  <input type="url" value={newNotification.url} onChange={(e) => setNewNotification({ ...newNotification, url: e.target.value })}
                    placeholder="https://cybev.io/..." className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-xl text-white focus:border-purple-500 focus:outline-none" />
                </div>
                ...
                <div className="bg-gray-900 rounded-xl p-4">
                  ...
                  <div className="bg-gray-700 rounded-lg p-3">
                    <div className="flex items-start gap-3">
                      <div className="w-10 h-10 bg-gradient-to-br from-purple-500 to-pink-500 rounded-lg flex items-center justify-center"><Bell className="w-5 h-5 text-white" /></div>
                      <div>
                        <p className="text-white font-medium">{newNotification.title || 'Notification Title'}</p>
                        <p className="text-gray-400 text-sm">{newNotification.body || 'Notification message...'}</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={() => setShowComposer(false)} className="flex-1 px-4 py-3 bg-gray-700 text-white rounded-xl hover:bg-gray-600">Cancel</button>
        ...
        <div className="bg-gray-800/50 rounded-2xl border border-purple-500/20 overflow-hidden">
          ...
          <div className="divide-y divide-gray-700/50">
            {loading ? (
              ...
              <div key={notif.id} className="p-4 hover:bg-gray-700/30">
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div className="flex gap-2">
                    <button className="p-2 hover:bg-gray-700 rounded-lg"><Eye className="w-4 h-4 text-gray-400" /></button>
                    {notif.status === 'scheduled' && <button className="p-2 hover:bg-gray-700 rounded-lg"><Edit className="w-4 h-4 text-gray-400" /></button>}
                    {notif.status === 'scheduled' && <button className="p-2 hover:bg-gray-700 rounded-lg"><Trash2 className="w-4 h-4 text-red-400" /></button>}
                  </div>
                </div>
