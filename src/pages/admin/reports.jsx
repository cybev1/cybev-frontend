...
const REPORT_TYPES = {
  ...
  misinformation: { label: 'Misinformation', color: 'bg-purple-500/20 text-purple-600' },
    ...
    <AppLayout>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center gap-4 mb-8">
          <div className="w-14 h-14 bg-red-500 rounded-2xl flex items-center justify-center"><Flag className="w-7 h-7 text-gray-900" /></div>
          <div><h1 className="text-2xl font-bold text-gray-900">Reports & Flags</h1><p className="text-gray-500">Review and manage user reports</p></div>
        </div>
        ...
        <div className="flex gap-2 mb-6">
          {['pending', 'resolved', 'dismissed', 'all'].map((f) => (
            <button key={f} onClick={() => setFilter(f)} className={`px-4 py-2 rounded-lg capitalize ${filter === f ? 'bg-purple-500 text-white' : 'bg-white text-gray-500 hover:bg-gray-100'}`}>{f}</button>
          ))}
        </div>
        ...
        <div className="space-y-4">
          {loading ? (
            <div className="bg-white/50 rounded-2xl p-12 flex justify-center"><Loader2 className="w-8 h-8 text-purple-500 animate-spin" /></div>
          ) : reports.map((report) => (
            <div key={report._id} className="bg-white/50 rounded-2xl border border-gray-200 p-6">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-gray-500 text-sm capitalize">{report.contentType}</span>
                  <span className={`px-2 py-0.5 rounded text-xs ${report.status === 'pending' ? 'bg-yellow-500/20 text-yellow-400' : report.status === 'resolved' ? 'bg-green-500/20 text-green-400' : 'bg-gray-500/20 text-gray-500'}`}>
                    {report.status}
                  </span>
                </div>
              ...
              <div className="bg-gray-50 rounded-xl p-4 mb-4">
                <p className="text-gray-600 line-clamp-2">{report.contentPreview}</p>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-4 mb-4 text-sm">
                ...
                <div>
                  ...
                  <p className="text-gray-900">@{report.reportedUser.username}</p>
                </div>
              </div>
              ...
              {report.status === 'pending' && (
                <div className="flex gap-3 pt-4 border-t border-gray-200">
                  <button onClick={() => handleAction(report._id, 'view')} className="flex items-center gap-2 px-4 py-2 bg-gray-100 text-gray-900 rounded-lg hover:bg-gray-600"><Eye className="w-4 h-4" />View Content</button>
                  <button onClick={() => handleAction(report._id, 'remove')} className="flex items-center gap-2 px-4 py-2 bg-red-500 text-gray-900 rounded-lg hover:bg-red-600"><Trash2 className="w-4 h-4" />Remove Content</button>
                  <button onClick={() => handleAction(report._id, 'ban')} className="flex items-center gap-2 px-4 py-2 bg-red-600 text-gray-900 rounded-lg hover:bg-red-700"><Ban className="w-4 h-4" />Ban User</button>
                  <button onClick={() => handleAction(report._id, 'dismiss')} className="flex items-center gap-2 px-4 py-2 bg-gray-600 text-gray-900 rounded-lg hover:bg-gray-500"><XCircle className="w-4 h-4" />Dismiss</button>
                </div>
              )}
