  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white dark:bg-white rounded-2xl shadow-xl max-w-md w-full p-8 text-center">
          {status === 'verifying' && (
            <>
              ...
              <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-900 mb-2">Verifying Payment</h1>
          ...
          {status === 'success' && (
            <>
              ...
              <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-900 mb-2">Domain Registered! 🎉</h1>
              <p className="text-gray-500 mb-6">Your domain is now active and configured.</p>
              {domain && (
                <div className="bg-gray-50 dark:bg-gray-700 rounded-xl p-4 mb-6">
                  <div className="flex items-center justify-center gap-2 mb-3"><Globe className="w-5 h-5 text-purple-600" /><span className="font-bold text-lg text-gray-900 dark:text-gray-900">{domain.domain}</span></div>
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div><span className="text-gray-500">Status</span><p className="font-medium text-green-600">Active</p></div>
                    <div><span className="text-gray-500">Expires</span><p className="font-medium text-gray-900 dark:text-gray-900">{domain.expiresAt ? new Date(domain.expiresAt).toLocaleDateString() : 'N/A'}</p></div>
                  </div>
                  {domain.dnsConfigured && <div className="mt-3 pt-3 border-t dark:border-gray-300"><span className="text-sm text-green-600 flex items-center justify-center gap-1"><CheckCircle className="w-4 h-4" />DNS configured</span></div>}
                </div>
              )}
              <div className="space-y-3">
                <button onClick={() => router.push('/settings/domains')} className="w-full py-3 bg-purple-600 text-gray-900 rounded-lg font-medium flex items-center justify-center gap-2">Manage Domain<ArrowRight className="w-4 h-4" /></button>
                <button onClick={() => router.push('/studio/sites/new')} className="w-full py-3 border dark:border-gray-300 text-gray-700 dark:text-gray-600 rounded-lg font-medium">Create Website</button>
              </div>
            </>
          )}
          ...
          {status === 'failed' && (
            <>
              ...
              <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-900 mb-2">Payment Failed</h1>
              ...
              <div className="space-y-3">
                <button onClick={() => router.push('/settings/domains')} className="w-full py-3 bg-purple-600 text-gray-900 rounded-lg font-medium">Try Again</button>
                <button onClick={() => router.push('/feed')} className="w-full py-3 border dark:border-gray-300 text-gray-700 dark:text-gray-600 rounded-lg font-medium">Back to Feed</button>
              </div>
            </>
          )}
        </div>
