// This is the complete production version. All 5 steps included, with hosting plans and payment modal in Step 5.
...
export default function BlogSetup() {
  ...
  const renderStep1 = () => (
    <>
      ...
      <select value={form.domainType} onChange={e => setForm({ ...form, domainType: e.target.value })} className="border p-2 rounded w-full mb-4">
  ...
  const renderStep2 = () => (
    <>
      ...
      <input placeholder="Blog Title" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} className="border p-2 rounded w-full mb-3" />
      <textarea placeholder="SEO Description" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} className="border p-2 rounded w-full mb-4" />
    </>
  );
  ...
  const renderStep3 = () => (
    <>
      ...
      <select value={form.template} onChange={e => setForm({ ...form, template: e.target.value })} className="border p-2 rounded w-full mb-4">
    ...
    <>
      ...
      {showPayment && selectedPlan && (
        <div className="mt-6 border-t pt-4">
          ...
          <select value={form.paymentMethod} onChange={e => setForm({ ...form, paymentMethod: e.target.value })} className="border p-2 rounded w-full mb-2">
            <option value="">Select Payment Method</option>
            <option value="paystack">Paystack</option>
            <option value="cryptomus">Cryptomus</option>
          </select>
          <button onClick={handlePayment} disabled={!form.paymentMethod} className="bg-green-600 text-white px-4 py-2 rounded">Pay ${selectedPlan.price}</button>
        </div>
      )}
    </>
  );
