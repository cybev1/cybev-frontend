...
export default function BlogSetup() {
  ...
  const renderStep5 = () => {
    const plans = [
      { id: 'basic', name: 'Basic Plan', price: '$5/month', features: ['1 Website', '10GB SSD', 'Free SSL'] },
      { id: 'pro', name: 'Pro Plan', price: '$10/month', features: ['5 Websites', '50GB SSD', 'Priority Support'] },
      { id: 'premium', name: 'Premium Plan', price: '$20/month', features: ['Unlimited Sites', '200GB SSD', 'Daily Backup'] },
    ];

    return (
      <>
        <h1 className="text-2xl font-bold mb-4">Step 5: Hosting & Publish</h1>
        <p className="text-gray-600 mb-4">Choose a hosting plan or skip this step to publish with free hosting.</p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          {plans.map(plan => (
            <div key={plan.id} className="border p-4 rounded shadow">
              <h2 className="font-bold text-lg mb-1">{plan.name}</h2>
              <p className="text-blue-600 font-semibold mb-2">{plan.price}</p>
              <ul className="text-sm mb-3">
                {plan.features.map((feat, idx) => <li key={idx}>• {feat}</li>)}
              </ul>
              <button className="bg-blue-600 text-white px-4 py-2 rounded w-full">Select Plan</button>
            </div>
          ))}
        </div>
        <div className="text-center mt-6">
          <button
            onClick={() => alert('🎉 Blog published successfully using free hosting!')}
