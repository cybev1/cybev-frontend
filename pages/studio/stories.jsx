...
export default function Stories() {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      {/* Greeting Card */}
      <motion.div className="p-4 bg-white dark:bg-gray-800 rounded-2xl shadow-lg flex items-center space-x-4">
      ...
      <div className="bg-white dark:bg-gray-800 shadow rounded-2xl overflow-x-auto whitespace-nowrap p-4">
        ...
        <div className="flex space-x-4">
          {stories.map(s => (
            <motion.div key={s.id} className="inline-block" whileHover={{ scale: 1.05 }}>
              <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-blue-500 p-1">
                <img src={s.avatar || '/default-avatar.png'} alt={s.userName} className="w-full h-full object-cover"/>
              </div>
              <p className="text-center text-sm mt-1">{s.userName}</p>
            </motion.div>
          ))}
        </div>
      </div>
